...
なお、240分追跡した内、群れ本体を追跡した割合は実に96.5%である。  
```{r}
prop_females_all %>% 
...
## メス割合が50%未満も含む場合
prop_females_all %>% 
  filter(prop_female > 0) %>% 
  ggplot(aes(x = prop_female)) +
  geom_histogram(color = "black",
                 fill = "white",
                 size = 0.4) +
  theme_bw(base_size = 12) +
  labs(x = "Proportion of adult females confirmed to be present",
       y = "Frequency of observations (N)") +
  ...
  theme(aspect.ratio = 1,
        ...
        axis.text = element_text(family = "Times New Roman")) -> p_hist_group

p_hist_group

# ggsave("figures/p_hist_group.tif", p_hist_group,
#        width = 100, height = 100, dpi = 1500, units = "mm")
```
<br/>  
...
prop_females_all %>% 
  ...
  summarise(prop = mean(hontai),
            N = n(),
            sum = sum(hontai)) -> prop_maingroup

prop_maingroup
```
<br/>  

続いて、前日確認された個体が次の日にも確認される割合を調べます。少ない日でも前日確認された個体の60%が次の日も確認されました。    
```{r}
no_female_over0.5 %>% 
  select(groupID) %>% 
...
female_pre_long %>% 
  left_join(female_pre_long %>% 
              ...
              select(date, femaleID, presence_pre)) %>% 
  drop_na(presence_pre) %>% 
  filter(presence_pre == 1) %>% 
  mutate(presence_successive = if_else(presence_pre == 1 & presence == 1, 1, 0)) %>% 
  group_by(date, groupID, study_period) %>% 
  summarise(prop = mean(presence_successive)) %>% 
  ungroup() -> prop_presence_successive

prop_presence_successive
```
<br/>  
...
m_female_prop <- brm(data = no_female_over0.5 %>% mutate(date=as.factor(date)),
                     no_female|trials(max_female) ~ study_period,
...
#### 結果の確認  
まず、モデルの結果は以下の通り。収束の問題は内容。有向サンプルサイズは6000を上回っている。   
```{r}
