...
まず、全期間のデータを用いたモデルについて。残差の自己相関をプロットすると、明確なパターンは見られなかった。また、Ljung-Box検定で3時点前までの残差との自己相関を検定したところ、有意な自己相関を示したのはKohとNtrだけだった。     
```{r, fig.dim = c(10,10)}
...
model_parameters(m_female_in_m) %>% 
  ...
  bold(j=c(1,2,3),i = c(4,5, 10, 11, 13),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:5,part = "all",align = "center") -> table_female_in_m
...
model_parameters(m_female_in_m_bf21) %>% 
  ...
  font(part = "body", j=2:5, fontname = "Times New Roman") %>% 
  font(part = "body", j=1, fontname = "Yu Gothic") %>% 
  bold(j=c(1,2,3),i = c(4,5),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:5,part = "all",align = "center") -> table_female_in_m_bf21
...
cont_female_in_m_TYstate %>% 
  ...
  facet_rep_wrap(~type,
                 repeat.tick.labels = TRUE,
                 scales = "free_y") -> p_female_in_m_TYcsi
...
```{r, fig.dim = c(12,5)}
cont_female_in_m_ITstate <- estimate_contrasts(m_female_in_m_TYstate,
                                               contrast = "IT_state",
                                               at = "CSI_IT_std",
                                               length = 50)

cont_female_in_m_ITstate %>% 
  mutate(type = str_c(Level1, " vs ", Level2)) %>% 
  mutate(type = str_replace_all(type, "TY_", "")) %>% 
  # mutate(type = fct_relevel(type, "absent vs stay",
  #                           "leave vs stay", "leave vs absent")) %>% 
  ...
  facet_rep_wrap(~type,
                 repeat.tick.labels = TRUE,
                 scales = "free_y") -> p_female_in_m_ITcsi
