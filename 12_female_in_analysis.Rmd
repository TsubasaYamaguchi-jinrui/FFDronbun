...
## データの加工  
### オスの出入り情報  
まず、オスの出入り情報について算出する。第\@ref(s10-2-1-3)でやったように4通りの場合を考える。  
```{r}
TYIT_presence_pre %>% 
...
続いて、2021年までのデータを用いたモデルをもとに、*IT*とのCSIに応じてオッズ比の対数をとったものがどのように変わるかを図示する。*TY*と親密度が高いメスは同じ日に群れに戻る確率が高いことが分かる。    
