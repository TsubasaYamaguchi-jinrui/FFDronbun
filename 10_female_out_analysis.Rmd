...
female_out_summary %>% 
  ...
  scale_fill_manual(values = c("grey28", "white"))+
...
一方で、*IT*の側から見ると9回中3回で、全体の33.33%だった。    
```{r}
male_out %>% 
  drop_na() %>% 
  mutate(TYIT_out = ifelse(TY_out == "1" & IT_out == "1",1,0)) %>% 
...
m_female_out_an <-  brm(data = female_out_an %>% 
                        ...
                        female_out ~ rate_agg_all_std + TY_out*CSI_TY_std +
                         IT_out*CSI_IT_std + ntm_std + est_std + prop_std +
...
```{r, echo = FALSE}
model_parameters(m_female_out_an, dispersion = TRUE, ci = 0.95) %>% 
  data.frame() %>% 
  ...
  mutate(Parameter = fct_relevel(Parameter,
                                  ...
                                  "b_kin_01",
                                  "b_rate_agg_all_std", "b_ntm_std", "b_est_std",
                                   "b_prop_std",
  ...
  bold(j=c(1,2,3,4,5),i = c(4,8, 15),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_female_out_an
...
```{r}
random_parameters(m_female_out_an)
```


続いて、2021年のデータを除いたモデルについて確認する。少しの違いはあるが、ITがかかわる変数については変化がない。  
```{r, echo = FALSE}
model_parameters(m_female_out_an_no21, dispersion = TRUE, ci = 0.95) %>% 
  data.frame() %>% 
  select(1,2,3,5,6, 7,8,9) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",CI_low),",",sprintf("%.2f",CI_high),"]")) %>% 
  select(c(1,2,3, 9), everything()) %>% 
  ...
  font(part = "body", j=2:7, fontname = "Times New Roman") %>% 
  font(part = "body", j=1, fontname = "Yu Gothic") %>% 
  bold(j=1:5,i = c(4, 8, 10, 11),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_female_out_an_no21
...
推定されたランダム効果の標準偏差は以下の通り。  
```{r}
random_parameters(m_female_out_an_no21)
```
...
###### メス同士の血縁度との関連  
最後に、血縁のメスが群れを離れたか否かとメスが離れる確率についての結果を図示する。血縁のメスが群れを離れるほど、同じ日に群れを離れる確率が高くなることが分かる。なお、少なくとも分母(= 機会)が3回以上のデータのみを用いている。   
...
female_out_an %>% 
  ...
  labs(x = "Did kin females left the group?",
...
m_female_out_an_TYstate <-  brm(data = female_out_an_b %>% 
                         ...
                         IT_state*CSI_IT_std + ntm_std + est_std + prop_std +
                         infant_pre + age_std + rank_std + kin_01 +
                          study_period + (1|femaleID) + (1|date),
                family = "bernoulli",
...
2021年のデータを除いたモデルでは、Hotのみで自己相関が有意になった。    
```{r, fig.dim = c(10,10)}
...
```{r, echo = FALSE}
model_parameters(m_female_out_an_TYstate, dispersion = TRUE, ci = 0.95) %>% 
  data.frame() %>% 
  select(1,2,3,5,6, 7,8,9) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",CI_low),",",sprintf("%.2f",CI_high),"]")) %>% 
  select(c(1,2,3, 9), everything()) %>% 
  ...
  font(part = "body", j=2:7, fontname = "Times New Roman") %>% 
  font(part = "body", j=1, fontname = "Yu Gothic") %>% 
  bold(j= 1:5,i = c(2,3,6,7,12),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_female_out_an_TYstate
...
ランダム効果の標準偏差は以下の通り。  
```{r}
random_parameters(m_female_out_an_TYstate)
```

続いて、2021年を除いたモデルの結果を確認する。結果はほとんど変わらず。    
```{r, echo = FALSE}
model_parameters(m_female_out_an_TYstate_no21, dispersion = TRUE, ci = 0.95) %>% 
  data.frame() %>% 
  select(1,2,3,5,6, 7,8,9) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",CI_low),",",sprintf("%.2f",CI_high),"]")) %>% 
  select(c(1,2,3, 9), everything()) %>% 
  ...
  font(part = "body", j=2:7, fontname = "Times New Roman") %>% 
  font(part = "body", j=1, fontname = "Yu Gothic") %>% 
  bold(j=c(1,2,3,4,5),i = c(2,3,5,10, 12, 13),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_female_out_an_TYstate_no21
...
ランダム効果の標準偏差は以下の通り。  
```{r}
random_parameters(m_female_out_an_TYstate_no21)
```
...
2021年を除くモデルについては、有意な自己相関はHotのみで見られた。      
```{r, fig.dim = c(10,10)}
...
結果は以下の通り。まずは全期間を含むデータについて確認する。  
```{r, echo = FALSE}
model_parameters(m_female_out_an_TY4cat, dispersion = TRUE, ci = 0.95) %>% 
  data.frame() %>% 
  select(1,2,3,5,6, 7,8,9) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",CI_low),",",sprintf("%.2f",CI_high),"]")) %>% 
  select(c(1,2,3, 9), everything()) %>% 
  ...
  font(part = "body", j=2:7, fontname = "Times New Roman") %>% 
  font(part = "body", j=1, fontname = "Yu Gothic") %>% 
  bold(j=c(1,2,3,4,5),i = c(2,4,8, 9, 16),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_female_out_an_TY4cat
...
ランダム効果に関するパラメータは以下の通り。  
```{r}
random_parameters(m_female_out_an_TY4cat)
```

続いて、2021年を除いたモデルの結果を確認する。ITに関する推定結果にわずかな違いがあるよう。ITが群れに戻った時にもメスが群れを離れる確率が有意に増加するよう。      
```{r, echo = FALSE}
model_parameters(m_female_out_an_TY4cat_no21, dispersion = TRUE, ci = 0.95) %>% 
  data.frame() %>% 
  select(1,2,3,5,6, 7,8,9) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",CI_low),",",sprintf("%.2f",CI_high),"]")) %>% 
  select(c(1,2,3, 9), everything()) %>% 
  ...
  font(part = "body", j=2:7, fontname = "Times New Roman") %>% 
  font(part = "body", j=1, fontname = "Yu Gothic") %>% 
  bold(j=c(1,2,3,4,5),i = c(2,4,6, 7, 13, 16),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_female_out_an_TY4cat_no21
...
ランダム効果に関するパラメータは以下の通り。  
```{r}
random_parameters(m_female_out_an_TY4cat_no21)
```
...
モデルは以下のように実行する。*IT*の出入りとCSIの交互作用は、*IT*が出たというデータ数が非常に少ないので変数に入れない。     
```{r} 
m_female_out_es <-  brm(data = female_out_es,
                        female_out ~ rate_agg_all_std + TY_out*CSI_TY_std + prop_std + 
                         IT_out + CSI_IT_std  + age_std + rank_std + kin_01 + ntm_std + est_std +
                          study_period + (1|femaleID) + (1|date),
                family = "bernoulli",
                prior = c(prior(student_t(4,0,20),class = Intercept),
...
m_female_out_es_bf21 <-  brm(data = female_out_es_no21,
                ...
                prior(student_t(4,0,15), class = b),
                prior(student_t(4,0,10), class = sd)),
  iter = 11000, warmup = 1000, seed = 120,
  control=list(adapt_delta = 0.999, max_treedepth = 15),
  backend = "cmdstanr",
...
2021年を除いたモデルについても見てみる。有意な自己相関を示した個体はTamだけだった。   
```{r, fig.dim = c(10,10)}
...
```{r, echo = FALSE}
model_parameters(m_female_out_es, dispersion = TRUE,
                 ci = 0.95) %>% 
  data.frame() %>% 
  select(1,2,3,5,6, 7,8,9) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",CI_low),",",sprintf("%.2f",CI_high),"]")) %>% 
  ...
  font(part = "body", j=1, fontname = "Yu Gothic") %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_female_out_es
...
ランダム効果に関するパラメータは以下の通り。  
```{r}
random_parameters(m_female_out_es)
```

2021年を除いたモデルでも同様。ただし、かなりデータの推定幅が大きいようだ。  
```{r, echo = FALSE}
model_parameters(m_female_out_es_bf21, dispersion = TRUE,
                 ci = 0.95) %>% 
  data.frame() %>% 
  select(1,2,3,5,6, 7,8,9) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",CI_low),",",sprintf("%.2f",CI_high),"]")) %>% 
  ...
  font(part = "body", j=1, fontname = "Yu Gothic") %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_female_out_es_bf21
...
ランダム効果に関するパラメータは以下の通り。推定された標準偏差がかなり大きい。    
```{r}
random_parameters(m_female_out_es_bf21)
```
...
```{r}
## オスの攻撃との交互作用なし
### IT_stateとCSI_IT_stdの交互作用はVIFが高いため除外  
m_female_out_es_TYstate <-  brm(data = female_out_es_b %>% 
                                  mutate(date = as.factor(date)),
                         female_out ~ rate_agg_all_std + TY_state*CSI_TY_std +
                         IT_state + CSI_IT_std + ntm_std + prop_std + est_std + 
...
model_parameters(m_female_out_es_TYstate,
  ...
  font(part = "body", j=2:7, fontname = "Times New Roman") %>% 
  font(part = "body", j=1, fontname = "Yu Gothic") %>% 
  bold(j=c(1,2,3,4,5),i = c(12),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_female_out_es_TYstate
...
model_parameters(m_female_out_es_TYstate_bf21,
  ...
  font(part = "body", j=2:7, fontname = "Times New Roman") %>% 
  font(part = "body", j=1, fontname = "Yu Gothic") %>% 
  bold(j=c(1,2,3,4,5),i = c(10),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_female_out_es_TYstate_bf21
...
###### 多重比較  
*IT*の動向については3水準あるので、事後検定として多重比較を行う。いずれも差は有意でない。    
```{r}
estimate_contrasts(m_female_out_es_TYstate,
                   contrast = "IT_state")
```
