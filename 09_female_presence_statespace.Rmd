...
male_time_18m <- group_all %>% 
  ...
  dplyr::select(date, groupID, TY, IT)  
...
male_time_19m <- read_excel("../Data/data/2019mating/2019mating_raw.xlsx",
  ...
  dplyr::select(maleID, date, groupID, presence_1h) %>% 
...
post_p_female_ss2_ri %>% 
  dplyr::select(iter, name, pred) %>% 
  pivot_wider(names_from = iter, 
              values_from = pred) %>% 
  dplyr::select(-name) %>% 
  bind_cols(nadrop_no_female2) %>% 
  dplyr::select(1:1000, no_female) %>% 
...
pd_female_ss2_ri <- p_direction(draws_female_ss2_ri[,2:11])

post_summary_female_ss2_ri %>% 
  filter(str_detect(variable, "b")) %>% 
  dplyr::select(1, 3,5,8,9,10, 12) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",`2.5%`),",",sprintf("%.2f",`97.5%`),"]")) %>% 
  mutate(PD = str_c(round(pd_female_ss2_ri$pd*100,2),"%")) %>% 
  dplyr::select(c(1,2,3,8,9), everything()) %>% 
  dplyr::select(-6, -7) %>% 
  rename("Parameter" = 1,
         "Median" = 2,
         "MAD" = 3,
  ...
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_female_ss2_ri

table_female_ss2_ri

# save_as_image(table_female_ss2_ri, "tables/table_female_ss2_ri.tiff",
#               res = 600)
```
...
post_p_female_ss_bf21nm_m <- draws_female_ss_bf21nm_m %>% 
  dplyr::select(starts_with("p")) %>% 
  dplyr::select(-phi) %>% 
  dplyr::select(any_of(obs_no_bf21nm_m)) %>% 
  mutate(iter = 1:n())  %>% 
  pivot_longer(cols = starts_with("p"),
               values_to = "p") %>% 
...
post_p_female_ss_bf21nm_nm <- draws_female_ss_bf21nm_nm %>% 
  dplyr::select(starts_with("p")) %>% 
  dplyr::select(-phi) %>% 
  dplyr::select(any_of(obs_no_bf21nm_nm)) %>% 
  mutate(iter = 1:n())  %>% 
  pivot_longer(cols = starts_with("p"),
               values_to = "p") %>% 
...
```{r}
# ベータ二項分布モデル
# stan_female_ss_trend <- cmdstan_model("stanfile/m_female_ss_trend_bb_ri.stan")
# 
...
有効サンプルサイズも少なくとも7000程度ある。  
```{r, fig.height = 4.5}
post_summary_female_ss_trend <- m_female_ss_trend$summary(
  variables = c("mu","p","b","lp__","s_t","s_w","mu_err","phi"),
  posterior::default_summary_measures(),
  lower = ~quantile(., 0.025, na.rm = TRUE),
  upper = ~quantile(., 0.975, na.rm = TRUE),
...
postmean_female_ss_trend <- post_summary_female_ss_trend %>% 
  filter(str_detect(variable, "p\\[")) %>% 
  bind_cols(nadrop_no_female_trend) %>% 
  mutate(resid = prop_female - mean) 
...
post_p_female_ss_trend <- draws_female_ss_trend %>% 
  ...
  pivot_longer(cols = starts_with("p"),
               values_to = "p") %>% 
  mutate(max_female = rep(nadrop_no_female_trend$max_female, 
                          times = nrow(draws_female_ss_trend))) %>% 
  mutate(phi = rep(draws_female_ss_trend$phi, each = nrow(nadrop_no_female_trend))) %>% 
  filter(iter %in% sample_n) %>% 
  mutate(pred = rbetabinom(nrow(.), size = max_female, prob = p, theta = phi))
...
post_p_female_ss_trend %>% 
  ...
  pivot_longer(everything(),
               names_to = "n_draw", 
               values_to = "post_pred") %>% 
  mutate(type = ifelse(str_detect(n_draw, "y_"),"y","y_pred")) -> post_pred_female_ss_trend_long

## 図示  
post_pred_female_ss_trend_long %>% 
  ggplot(aes(x = post_pred))+
  geom_line(stat = "density",
            aes(group = n_draw,
                color = type,
...
pd_female_ss_trend <- p_direction(draws_female_ss_trend[,2:11])
...
post_summary_female_ss_trend %>% 
  ...
  font(part = "body", j=2:7, fontname = "Times New Roman") %>% 
  font(part = "body", j=1, fontname = "Yu Gothic") %>% 
  bold(j=c(1,2,3,4,5),i = c(1, 3),bold = TRUE) %>% 
  theme_zebra() %>% 
...
as_draws_df(draws_female_ss_trend, .nchains = 4) %>% 
  dplyr::select(4:9) %>% 
  mutate(`m18 - m19` = `b[3]` - `b[4]`,
         `m18 - m20` = `b[3]` - `b[5]`,
         `m18 - nm19` = `b[3]` - `b[6]`,
...
fit_female_ss_trend %>% 
  ...
  facet_rep_wrap(~study_period,
                 repeat.tick.labels = TRUE,
                 scales = "free_x",
                 ncol = 2) -> p_female_ss_trend_all_m

p_female_ss_trend_all_m

# ggsave("figures/p_female_ss_trend_all_m.png",p_female_ss_trend_all_m,
#        dpi = 600, units = "mm",
#        width = 400, height = 180)
  ...
  theme(axis.text.x = element_text(angle = 45,
    ...
    facetted_pos_scales(x = list(
                study_period == "nm20" ~ scale_x_date(breaks = seq(as_date("2020-03-12"),
                                                      as_date("2020-03-29"),
                                                     by = "1 days"),
                                  labels = date_format("%m/%d")))) -> p_female_ss_trend_all_nm

p_female_ss_trend_all_nm

# ggsave("figures/p_female_ss_trend_all_nm.png",p_female_ss_trend_all_nm,
#        dpi = 600, units = "mm",
#        width = 400, height = 180)
```
<br/>  
...
有効サンプルサイズも少なくとも7000程度ある。  
```{r, fig.height = 4.5}
post_summary_female_ss_trend_bf21nm <- m_female_ss_trend_bf21nm$summary(
  variables = c("mu","p","b","lp__","s_t","s_w","mu_err","phi"),
  posterior::default_summary_measures(),
  lower = ~quantile(., 0.025, na.rm = TRUE),
  upper = ~quantile(., 0.975, na.rm = TRUE),
...
post_p_female_ss_trend_bf21nm <- draws_female_ss_trend_bf21nm %>% 
  ...
  mutate(phi = rep(draws_female_ss_trend_bf21nm$phi, each = nrow(nadrop_no_female_trend_bf21nm))) %>% 
  filter(iter %in% sample_n) %>% 
  mutate(pred = rbetabinom(nrow(.), size = max_female, prob = p, theta = phi))
...
post_p_female_ss_trend_bf21nm %>% 
  ...
  pivot_longer(everything(),
               names_to = "n_draw", 
               values_to = "post_pred") %>% 
  mutate(type = ifelse(str_detect(n_draw, "y_"),"y","y_pred")) -> post_pred_female_ss_trend_bf21nm_long

## 図示  
post_pred_female_ss_trend_bf21nm_long %>% 
  ggplot(aes(x = post_pred))+
  geom_line(stat = "density",
            aes(group = n_draw,
                color = type,
...
pd_female_ss_trend_ri_bf21 <- p_direction(draws_female_ss_trend_bf21nm[,2:9])

post_summary_female_ss_trend_bf21nm %>% 
  filter(str_detect(variable, "b")) %>% 
  dplyr::select(1, 3,5,8,9,10, 12) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",`2.5%`),",",sprintf("%.2f",`97.5%`),"]")) %>% 
  mutate(PD = str_c(round(pd_female_ss_trend_ri_bf21$pd*100,2),"%")) %>% 
  dplyr::select(c(1,2,3,8,9), everything()) %>% 
  dplyr::select(-6, -7) %>% 
  rename("Parameter" = 1,
         "Median" = 2,
...
as_draws_df(draws_female_ss_trend_bf21nm, .nchains = 4) %>% 
  dplyr::select(4:9) %>% 
  mutate(`m18 - m19` = `b[3]` - `b[4]`,
         `m18 - m20` = `b[3]` - `b[5]`,
         `m18 - nm19` = `b[3]` - `b[6]`,
...
fit_female_ss_trend_bf21nm %>% 
  ...
  facet_rep_wrap(~study_period,
                 repeat.tick.labels = TRUE,
                 scales = "free_x",
                 ncol = 2) -> p_female_ss_trend_bf21nm_all_TY_m

p_female_ss_trend_bf21nm_all_TY_m

# ggsave("figures/p_female_ss_trend_bf21nm_all_TY_m.png",p_female_ss_trend_bf21nm_all_TY_m,
#        dpi = 600, units = "mm",
#        width = 400, height = 180)
  ...
  theme(axis.text.x = element_text(angle = 45,
    ...
    facetted_pos_scales(x = list(
                study_period == "nm20" ~ scale_x_date(breaks = seq(as_date("2020-03-12"),
                                                      as_date("2020-03-29"),
                                                     by = "1 days"),
                                  labels = date_format("%m/%d")))) -> p_female_ss_trend_bf21nm_all_TY_nm

p_female_ss_trend_bf21nm_all_TY_nm

# ggsave("figures/p_female_ss_trend_bf21nm_all_TY_nm.png",p_female_ss_trend_bf21nm_all_TY_nm,
#        dpi = 600, units = "mm",
#        width = 400, height = 180)
```
<br/>  
...
## DHARMaでのチェック  
```{r}
# draws_female_ss_p <-  draws_female_ss_trend %>%
#   dplyr::select(starts_with("p")) %>%
#   dplyr::select(-phi)
# 
...
#   pivot_longer(cols = starts_with("p"),
#                values_to = "p") %>% 
#   mutate(max_female = rep(nadrop_no_female_trend$max_female, times = 20000)) %>%
#   mutate(phi = rep(draws_female_ss_trend$phi, each = nrow(nadrop_no_female_trend)))
# 
...
#   observedResponse = nadrop_no_female_trend$no_female,
#   fittedPredictedResponse = map_vec(draws_female_ss_p , median),
