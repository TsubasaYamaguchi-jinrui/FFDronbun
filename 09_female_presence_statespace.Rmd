...
# 
# m_female_ss <- stan_female_ss$sample(
...
# )
# 
# m_female_ss$save_object("model/m_female_ss.rds")
# 
# ## ランダム効果なし  
...
m_female_ss <- readRDS("model/m_female_ss.rds")  
```

##### モデルの診断  
Rhatは小さく、収束に問題ないよう。    
```{r, fig.height = 4.5}
rhat_female_ss <- m_female_ss %>% 
  bayesplot::rhat()

data.frame(Rhat = rhat_female_ss) %>% 
  ggplot(aes(x = Rhat))+
...
```{r, fig.height = 4.5}
post_summary_female_ss <- m_female_ss$summary(variables =
...
残差の自己相関は消えている。  
```{r}
postmean_female_ss <- post_summary_female_ss %>% 
  filter(str_detect(variable, "p\\[")) %>% 
  bind_cols(no_female_over0.5_b2) %>% 
  mutate(resid = prop_female - mean) 

acf_female_ss <- acf(postmean_female_ss$resid,
                         na.action = na.pass,
                         plot = F)

data.frame(Lag = acf_female_ss$lag[,,1],
           acf = acf_female_ss$acf[,,1]) %>% 
  ...
  labs(y = "Autocorrelation of residuals") -> p_acf_female_ss  

p_acf_female_ss

# ggsave("figures/p_acf_female_ss.png", p_acf_female_ss,
...
```{r, fig.height = 4.5}
draws_female_ss <- m_female_ss$draws() %>% 
  as_draws_df(nchains = 4)

## pの事後分布から100個のサンプルを抽出してシミュレート    
set.seed(123)
sample_n <- sample(1:nrow(draws_female_ss), size = 1000,
                   replace = FALSE)

post_p_female_ss <- draws_female_ss %>% 
  ...
  pivot_longer(cols = starts_with("p"),
               values_to = "p") %>% 
  mutate(max_female = rep(nadrop_no_female2$max_female, times = nrow(draws_female_ss))) %>% 
  mutate(phi = rep(draws_female_ss$phi, each = nrow(nadrop_no_female2))) %>% 
...
post_summary_female_ss %>% 
  ...
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_female_ss

table_female_ss

# save_as_image(table_female_ss, "tables/table_female_ss.tiff",
#               res = 600)
```
...
```{r, height = 6}
fit_female_ss <- as_draws_df(draws_female_ss, .nchains = 4) %>% 
  select(contains("p[")) %>% 
  pivot_longer(everything(),
               names_to = "p",
               values_to = "post") %>% 
...
fit_female_ss %>% 
  ...
  facet_rep_wrap(~study_period,
                 ...
                 scales = "free_x") -> p_female_ss_TY_m

p_female_ss_TY_m

# ggsave("figures/p_female_ss_TY_m.png",p_female_ss_TY_m,
#        dpi = 600, units = "mm",
  ...
  theme(axis.text.x = element_text(angle = 45,
    ...
    facetted_pos_scales(x = list(
                study_period == "nm20" ~ scale_x_date(breaks = seq(as_date("2020-03-12"),
                                                      as_date("2020-03-29"),
                                                     by = "1 days"),
                                          labels = date_format("%m/%d")))) -> p_female_ss_TY_nm

p_female_ss_TY_nm

# ggsave("figures/p_female_ss_TY_nm.png",p_female_ss_TY_nm,
#        dpi = 600, units = "mm",
#        width = 400, height = 180)
```
<br/>  
...
```{r}
draw_ss_p <- as_draws_df(draws_female_ss, .nchains = 4) %>% 
  select(contains("p[")) %>% 
  pivot_longer(everything(),
               names_to = "p",
               values_to = "post") %>% 
...
draw_ss_p %>% 
  ...
  summarise(median_prob = median(post),
            lower_prob = quantile(post, 0.025),
            upper_prob = quantile(post, 0.975)) %>% 
  mutate(TY = as.factor(TY)) -> post_female_ss_TY
...
nadrop_no_female2 %>% 
  ...
  stat_boxplot(geom='errorbar',width=0.1,lwd= 1,
               color = "grey23")+
  geom_pointrange(data = post_female_ss_TY,
                aes(y = median_prob, ymin = lower_prob,ymax = upper_prob),
                  color = "grey85",size = 0.3,
                linetype = "dashed")+
  geom_errorbar(data = post_female_ss_TY,
                aes(y = median_prob, ymin = lower_prob,ymax = upper_prob),
  ...
  theme(aspect.ratio = 1,
        ...
        axis.title.y = element_text(family = "Times New Roman",
                                    size = 14),
        axis.title.x = element_text(family = "Times New Roman")) -> p_female_ss_TY

# ggsave("figures/p_female_ss_TY.png",p_female_ss_TY,
#         dpi = 600, units = "mm",
...
draw_ss_p %>% 
  ...
  summarise(median_prob = median(post),
            lower_prob = quantile(post, 0.025),
            upper_prob = quantile(post, 0.975)) %>% 
  mutate(IT = as.factor(IT)) -> post_female_ss_IT
...
nadrop_no_female2 %>% 
  ...
  stat_boxplot(geom='errorbar',width=0.1,lwd= 1,
               color = "grey23")+
  geom_pointrange(data = post_female_ss_IT,
                aes(y = median_prob, ymin = lower_prob,ymax = upper_prob),
                  color = "grey85",size = 0.3,
                linetype = "dashed")+
  geom_errorbar(data = post_female_ss_IT,
                aes(y = median_prob, ymin = lower_prob,ymax = upper_prob),
                color = "grey85",width = 0.1,size=0.3,
                linetype = "dashed")+
  ...
  theme(aspect.ratio = 1,
        ...
        axis.title.y = element_text(family = "Times New Roman",
                                    size = 14),
        axis.title.x = element_text(family = "Times New Roman")) -> p_female_ss_IT

# ggsave("figures/p_female_ss_IT.png",p_female_ss_IT,
#         dpi = 600, units = "mm",
#         width = 100, height = 100)
```

```{r, fig.height = 4}
p_female_ss_TY + p_female_ss_IT
```
...
pd_female_ss_ri_bf21 <- p_direction(draws_female_ss_bf21nm[,2:9])

post_summary_female_ss_bf21nm %>% 
  filter(str_detect(variable, "b")) %>% 
  select(1, 3,5,8,9,10, 12) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",`2.5%`),",",sprintf("%.2f",`97.5%`),"]")) %>% 
  mutate(PD = str_c(round(pd_female_ss_ri_bf21$pd*100,2),"%")) %>% 
  select(c(1,2,3,8,9), everything()) %>% 
  select(-6, -7) %>% 
  rename("Parameter" = 1,
         "Median" = 2,
  ...
  bold(j=c(1,2,3,4,5),i = c(1,2),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_female_ss_bf21nm

table_female_ss_bf21nm

# save_as_image(table_female_ss_bf21nm, "tables/table_female_ss_bf21nm.tiff",
#               res = 600)
```
...
```{r}
## 二項分布モデル
# stan_female_ss_trend <- cmdstan_model("stanfile/m_female_ss_trend_binom.stan")
# 
...
Rhatは小さく、収束に問題ないよう。    
```{r, fig.height = 4.5}
rhat_female_ss_trend <- m_female_ss_trend %>% 
  bayesplot::rhat()

data.frame(Rhat = rhat_female_ss_trend) %>% 
  ggplot(aes(x = Rhat))+
  geom_histogram(fill = "white",
                 color = "black")+
  theme_bw()+
...
有効サンプルサイズも少なくとも2000程度ある。  
```{r, fig.height = 4.5}
post_summary_female_ss_trend <- m_female_ss_trend$summary(variables =
                                                     c("mu","logit_p","b","lp__","s_t",
                                                       "mu_err","mu_ini"),
                                                posterior::default_summary_measures(),
                                                lower = ~quantile(., 0.025, na.rm = TRUE),
                                                upper = ~quantile(., 0.975, na.rm = TRUE),
...
残差の自己相関は消えている。  
```{r}
postmean_female_ss_trend <- post_summary_female_ss_trend %>% 
  filter(str_detect(variable, "logit_p\\[")) %>% 
  bind_cols(drop_na(no_female_over0.5_b2)) %>% 
  mutate(resid = prop_female - inv_logit_scaled(mean)) 

acf_female_ss_trend <- acf(postmean_female_ss_trend$resid,
                         na.action = na.pass,
                         plot = F)

data.frame(Lag = acf_female_ss_trend$lag[,,1],
           acf = acf_female_ss_trend$acf[,,1]) %>% 
  ggplot(aes(x = Lag, y = acf))+
  geom_segment(aes(yend = 0, xend = Lag))+
  theme_bw(base_size = 16)+
  theme(aspect.ratio = 1,
        ...
        axis.text.x = element_text(family = "Arial"),
        axis.text.y = element_text(family = "Arial"))+
  labs(y = "Autocorrelation of residuals") -> p_acf_female_ss_trend  

p_acf_female_ss_trend

# ggsave("figures/p_acf_female_ss_trend.png", p_acf_female_ss_trend,
#        width = 100, height = 100, dpi = 600, units = "mm")
```
<br/>  
...
事後分布からの予測分布と実測値の分布を比較しても大きな乖離はない。   
```{r, fig.height = 4.5}
draws_female_ss_trend <- m_female_ss_trend$draws() %>% 
  as_draws_df(nchains = 4)

## pの事後分布から100個のサンプルを抽出してシミュレート    
set.seed(123)
sample_n <- sample(1:nrow(draws_female_ss_trend), size = 1000,
                   replace = FALSE)

post_p_female_ss_trend <- draws_female_ss_trend %>% 
  select(starts_with("logit_p")) %>% 
  mutate(iter = 1:n())  %>% 
  pivot_longer(cols = starts_with("logit_p"),
               values_to = "logit_p") %>% 
  mutate(max_female = rep(nadrop_no_female2$max_female, times = nrow(draws_female_ss_trend))) %>% 
  filter(iter %in% sample_n) %>% 
  mutate(pred = rbinom(nrow(.), size = max_female, prob = inv_logit_scaled(logit_p)))

## 事後予測値を事後平均からシミュレート
post_p_female_ss_trend %>% 
  select(iter, name, pred) %>% 
  pivot_wider(names_from = iter, 
              values_from = pred) %>% 
  select(-name) %>% 
...
```{r}
post_summary_female_ss_trend %>% 
  filter(str_detect(variable, "b") | str_detect(variable, "^s_"))

pd_female_ss_trend <- p_direction(draws_female_ss_trend[,4:13])

post_summary_female_ss_trend %>% 
  filter(str_detect(variable, "b")) %>% 
  select(1, 3,5,8,9,10, 12) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",`2.5%`),",",sprintf("%.2f",`97.5%`),"]")) %>% 
  mutate(PD = str_c(round(pd_female_ss_trend$pd*100,2),"%")) %>% 
  select(c(1,2,3,8,9), everything()) %>% 
  select(-6, -7) %>% 
  rename("Parameter" = 1,
         "Median" = 2,
  ...
  bold(j=c(1,2,3,4,5),i = c(1,2),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_female_ss_trend

table_female_ss_trend

# save_as_image(table_female_ss_trend, "tables/table_female_ss_trend.tiff",
#               res = 600)
```

*TY*の有無をコントロールすると、調査期間間の効果の違いはない。つまり、調査期間間のメス確認割合の違いは*TY*の有無に大きく影響されていたことが分かった。  
```{r}
draws_female_ss_trend %>% 
  select(6:13) %>% 
  mutate(`m18 - m19` = `b[3]` - `b[4]`,
         `m18 - m20` = `b[3]` - `b[5]`,
         `m18 - m21` = `b[3]` - `b[6]`,
...
それでは、結果を図示する。まずは、時系列的に$p_t$の事後平均と95%確信区間を図示する。*TY*と*IT*の有無で点の色を変えている。交尾期と非交尾期は分けて図示する。      
```{r, height = 6}
fit_female_ss_trend <- as_draws_df(draws_female_ss_trend, .nchains = 4) %>% 
  select(contains("logit_p[")) %>% 
  pivot_longer(everything(),
               names_to = "logit_p",
               values_to = "post") %>% 
  drop_na() %>%
  ...
  summarise(mean_prob = median(post),
            ...
            upper_prob = quantile(post, 0.975)) %>% 
  ungroup() %>% 
  mutate(p2 = as.numeric(str_replace_all(logit_p, c("logit_p\\[" = "","\\]" = "")))) %>% 
  arrange(p2) %>% 
  bind_cols(nadrop_no_female2) %>% 
  mutate(TY = as.factor(TY),
         IT = as.factor(IT)) %>% 
...
fit_female_ss_trend %>% 
  ...
  facet_rep_wrap(~study_period,
                 repeat.tick.labels = TRUE,
                 scales = "free_x") -> p_female_ss_trend_TY_m

p_female_ss_trend_TY_m

# ggsave("figures/p_female_ss_trend_TY_m.png",p_female_ss_trend_TY_m,
#        dpi = 600, units = "mm",
#        width = 400, height = 180)
  ...
  theme(axis.text.x = element_text(angle = 45,
    ...
    facetted_pos_scales(x = list(
                study_period == "nm20" ~ scale_x_date(breaks = seq(as_date("2020-03-12"),
                                                      as_date("2020-03-29"),
                                                     by = "1 days"),
                                          labels = date_format("%m/%d")))) -> p_female_ss_trend_TY_nm

p_female_ss_trend_TY_nm

  # ggsave("figures/p_female_ss_trend_TY_nm.png",p_female_ss_trend_TY_nm,
  #        dpi = 600, units = "mm",
  #        width = 400, height = 180)
```
<br/>  
...
Rhatは小さく、収束に問題ないよう。    
```{r, fig.height = 4.5}
rhat_female_ss_trend_bf21nm <- m_female_ss_trend_bf21nm %>% 
  bayesplot::rhat()

data.frame(Rhat = rhat_female_ss_trend_bf21nm) %>% 
  ggplot(aes(x = Rhat))+
  geom_histogram(fill = "white",
                 color = "black")+
  theme_bw()+
...
有効サンプルサイズも少なくとも1200程度ある。  
```{r, fig.height = 4.5}
post_summary_female_ss_trend_bf21nm <- m_female_ss_trend_bf21nm$summary(variables =
                                                     c("mu","logit_p","b","lp__","s_t",
                                                       "mu_err","mu_ini"),
                                                posterior::default_summary_measures(),
                                                lower = ~quantile(., 0.025, na.rm = TRUE),
...
post_summary_female_ss_trend_bf21nm %>% 
  ggplot(aes(x = ess_bulk)) +
  geom_histogram(fill = "white",
                 color = "black")+
  theme_bw()+
...
残差の自己相関は消えている。  
```{r}
postmean_female_ss_trend_bf21nm <- post_summary_female_ss_trend_bf21nm %>% 
  filter(str_detect(variable, "logit_p\\[")) %>% 
  bind_cols(drop_na(no_female_over0.5_bf21nm)) %>% 
  mutate(resid = prop_female - inv_logit_scaled(mean)) 

acf_female_ss_trend_bf21nm <- acf(postmean_female_ss_trend_bf21nm$resid,
                         na.action = na.pass,
                         plot = F)

data.frame(Lag = acf_female_ss_trend_bf21nm$lag[,,1],
           acf = acf_female_ss_trend_bf21nm$acf[,,1]) %>% 
  ggplot(aes(x = Lag, y = acf))+
  geom_segment(aes(yend = 0, xend = Lag))+
  theme_bw(base_size = 16)+
  theme(aspect.ratio = 1,
        ...
        axis.text.x = element_text(family = "Arial"),
        axis.text.y = element_text(family = "Arial"))+
  labs(y = "Autocorrelation of residuals") -> p_acf_female_ss_trend_bf21nm  

p_acf_female_ss_trend_bf21nm

# ggsave("figures/p_acf_female_ss_trend_bf21nm.png", p_acf_female_ss_trend_bf21nm,
#        width = 100, height = 100, dpi = 600, units = "mm")
```
<br/>  
...
事後分布からの予測分布と実測値の分布を比較しても大きな乖離はない。   
```{r, fig.height = 4.5}
draws_female_ss_trend_bf21nm <- m_female_ss_trend_bf21nm$draws() %>% 
  as_draws_df(nchains = 4)

## pの事後分布から100個のサンプルを抽出してシミュレート    
set.seed(123)
sample_n <- sample(1:nrow(draws_female_ss_trend_bf21nm), size = 1000,
                   replace = FALSE)

post_p_female_ss_trend_bf21nm <- draws_female_ss_trend_bf21nm %>% 
  select(starts_with("logit_p")) %>% 
  mutate(iter = 1:n())  %>% 
  pivot_longer(cols = starts_with("logit_p"),
               values_to = "logit_p") %>% 
  mutate(max_female = rep(nadrop_no_female_trend_bf21nm$max_female, 
                          times = nrow(draws_female_ss_trend_bf21nm))) %>% 
  filter(iter %in% sample_n) %>% 
  mutate(pred = rbinom(nrow(.), size = max_female, prob = inv_logit_scaled(logit_p)))

## 事後予測値を事後平均からシミュレート
post_p_female_ss_trend_bf21nm %>% 
  select(iter, name, pred) %>% 
  pivot_wider(names_from = iter, 
              values_from = pred) %>% 
  select(-name) %>% 
  bind_cols(nadrop_no_female_trend_bf21nm) %>% 
  select(1:1000, no_female) %>% 
  rename(y_actual = no_female) %>% 
  pivot_longer(everything(),
               names_to = "n_draw", 
...
pd_female_ss_trend_bf21nm <- p_direction(draws_female_ss_trend_bf21nm[,4:11])

post_summary_female_ss_trend_bf21nm %>% 
  filter(str_detect(variable, "b")) %>% 
  select(1, 3,5,8,9,10, 12) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",`2.5%`),",",sprintf("%.2f",`97.5%`),"]")) %>% 
  mutate(PD = str_c(round(pd_female_ss_trend_bf21nm$pd*100,2),"%")) %>% 
  select(c(1,2,3,8,9), everything()) %>% 
  select(-6, -7) %>% 
  rename("Parameter" = 1,
         "Median" = 2,
  ...
  font(part = "body", j=2:7, fontname = "Times New Roman") %>% 
  font(part = "body", j=1, fontname = "Yu Gothic") %>% 
  bold(j=c(1,2,3,4,5),i = c(1),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_female_ss_trend_bf21nm

table_female_ss_trend_bf21nm

# save_as_image(table_female_ss_trend_bf21nm, "tables/table_female_ss_trend_bf21nm.tiff",
#               res = 600)
```

*TY*の有無をコントロールすると、調査期間間の効果の違いはない。つまり、調査期間間のメス確認割合の違いは*TY*の有無に大きく影響されていたことが分かった。  
```{r}
draws_female_ss_trend_bf21nm %>% 
  select(6:11) %>% 
  mutate(`m18 - m19` = `b[3]` - `b[4]`,
         `m18 - m20` = `b[3]` - `b[5]`,
         `m18 - nm19` = `b[3]` - `b[6]`,
...
それでは、結果を図示する。まずは、時系列的に$p_t$の事後平均と95%確信区間を図示する。*TY*と*IT*の有無で点の色を変えている。交尾期と非交尾期は分けて図示する。      
```{r, height = 6}
fit_female_ss_trend_bf21nm <- as_draws_df(draws_female_ss_trend_bf21nm, .nchains = 4) %>% 
  select(contains("logit_p[")) %>% 
  pivot_longer(everything(),
               names_to = "logit_p",
               values_to = "post") %>% 
...
fit_female_ss_trend_bf21nm %>% 
  ...
  facet_rep_wrap(~study_period,
                 repeat.tick.labels = TRUE,
                 scales = "free_x") -> p_female_ss_trend_bf21nm_TY_m

p_female_ss_trend_bf21nm_TY_m

# ggsave("figures/p_female_ss_trend_bf21nm_TY_m.png",p_female_ss_trend_bf21nm_TY_m,
#        dpi = 600, units = "mm",
#        width = 400, height = 180)
  ...
  theme(axis.text.x = element_text(angle = 45,
    ...
    facetted_pos_scales(x = list(
                study_period == "nm20" ~ scale_x_date(breaks = seq(as_date("2020-03-12"),
                                                      as_date("2020-03-29"),
                                                     by = "1 days"),
                                          labels = date_format("%m/%d")))) -> p_female_ss_trend_bf21nm_TY_nm

p_female_ss_trend_bf21nm_TY_nm

  # ggsave("figures/p_female_ss_trend_bf21nm_TY_nm.png",p_female_ss_trend_bf21nm_TY_nm,
  #        dpi = 600, units = "mm",
  #        width = 400, height = 180)
```
<br/>  
...
## DHARMaでのチェック  
```{r}
draws_female_ss_p <-  draws_female_ss %>%
  select(starts_with("p"))
...
```{r}
draws_female_ss_p <-  draws_female_ss_trend_bf21nm %>%
  select(starts_with("logit_p"))
...
draws_female_ss_m_p2 <-  draws_female_ss_p %>%
  ...
  pivot_longer(cols = starts_with("logit_p"),
               values_to = "logit_p") %>%
  mutate(p = inv_logit_scaled(logit_p)) %>% 
  mutate(max_female = rep(nadrop_no_female_bf21nm$max_female, times = 20000)) 
...
model.check_m <- createDHARMa(
  simulatedResponse = t(pred_sim),
  observedResponse = nadrop_no_female_bf21nm$no_female,
  fittedPredictedResponse = apply(t(pred_sim),1, mean),
  integerResponse = TRUE)
