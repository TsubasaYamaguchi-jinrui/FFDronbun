...
```{r, echo = FALSE}
model_parameters(m_agg_TYIT2, dispersion = TRUE) %>% 
  data.frame() %>% 
  ...
  bold(j=c(1,2,3,4,5),i = c(2,3,6,10:15, 17,18),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_agg_TYIT
...
結果は以下の通り。太字になっている変数は95%確信区間が0をまたいでおらず、有意な影響があるといえる。  
```{r, echo = FALSE}
model_parameters(m_agg_TYIT_bf21, dispersion = TRUE) %>% 
  data.frame() %>% 
  select(1,3,4,6, 7,8,9,10) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",CI_low),",",sprintf("%.2f",CI_high),"]")) %>% 
  select(c(1,2,3, 9), everything()) %>% 
  ...
  font(part = "body", j=2:7, fontname = "Segoe UI") %>% 
  font(part = "body", j=1, fontname = "Yu Gothic") %>% 
  bold(j=c(1,2,3,4,5),i = c(2,3,6,10,11,13,14,17),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_agg_TYIT_bf21
...
```{r, echo = FALSE}
model_parameters(m_agg_TYIT_es, dispersion = TRUE) %>% 
  data.frame() %>% 
  select(1,3,4,6, 7,8,9,10) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",CI_low),",",sprintf("%.2f",CI_high),"]")) %>% 
  select(c(1,2,3, 9), everything()) %>% 
  ...
  font(part = "body", j=2:7, fontname = "Segoe UI") %>% 
  font(part = "body", j=1, fontname = "Yu Gothic") %>% 
  bold(j=c(1,2,3,4,5),i = c(3, 9, 11, 14, 15),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_agg_TYIT_es
...
```{r, echo = FALSE}
model_parameters(m_agg_TYIT_an, dispersion = TRUE) %>% 
  data.frame() %>% 
  select(1,3,4,6, 7,8,9,10) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",CI_low),",",sprintf("%.2f",CI_high),"]")) %>% 
  select(c(1,2,3, 9), everything()) %>% 
  ...
  font(part = "body", j=2:7, fontname = "Segoe UI") %>% 
  font(part = "body", j=1, fontname = "Yu Gothic") %>% 
  bold(j=c(1,2,3,4,5),i = c(2,3,8,9,10,11, 12, 14, 15),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_agg_TYIT_an
...
```{r, echo = FALSE}
model_parameters(m_agg_TYIT_bf21_es, dispersion = TRUE) %>% 
  data.frame() %>% 
  select(1,3,4,6, 7,8,9,10) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",CI_low),",",sprintf("%.2f",CI_high),"]")) %>% 
  select(c(1,2,3, 9), everything()) %>% 
  ...
  font(part = "body", j=2:7, fontname = "Segoe UI") %>% 
  font(part = "body", j=1, fontname = "Yu Gothic") %>% 
  bold(j=c(1,2,3,4,5),i = c(3),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_agg_TYIT_bf21_es
...
続いて、非発情メスについては以下のとおりである。  
```{r, echo = FALSE}
model_parameters(m_agg_TYIT_bf21_an, dispersion = TRUE) %>% 
  data.frame() %>% 
  select(1,3,4,6, 7,8,9,10) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",CI_low),",",sprintf("%.2f",CI_high),"]")) %>% 
  select(c(1,2,3, 9), everything()) %>% 
  ...
  font(part = "body", j=2:7, fontname = "Segoe UI") %>% 
  font(part = "body", j=1, fontname = "Yu Gothic") %>% 
  bold(j=c(1,2,3,4,5),i = c(3,8, 10,11, 14),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:7,part = "all",align = "center") -> table_agg_TYIT_bf21_an
