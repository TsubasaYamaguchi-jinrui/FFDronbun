...
no_female_over0.5 %>% 
  ...
  mutate(rs_std = standardize(prop_rs),
         ...
         prop_IT_std = standardize(prop_IT)) -> female_presence_agg
```
...
```{r, echo = FALSE}
model_parameters(m_presence_agg) %>% 
  data.frame() %>% 
  select(1,3,5,6,8,9) %>% 
  ...
  bold(j=c(1,2,3),i = c(2,3,5, 6,7),bold = TRUE) %>% 
  theme_zebra() %>% 
  align(j=2:5,part = "all",align = "center") -> table_presence_agg
